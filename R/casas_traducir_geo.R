...
casas_f <- read_csv("../datos/houseprices/house-prices-train.csv") %>%
  dplyr::select(id = Id, tipo_zona = MSZoning, frente_lote = LotFrontage,
         area_lote = LotArea,
         ...
         precio = SalePrice)
pos <- read_csv("../datos/houseprices/geo_neighborhoods.csv") %>%
