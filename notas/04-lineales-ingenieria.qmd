...
Tradicionalmente, utilizamos métodos como mínimos cuadrados pasra encontrar los coeficientes 
$\beta$, en donde minimizamos el *error de entrenamiento*. En aprendizaje automático moderno,
se utilizan otros métodos como descenso estocástico, que pueden ser más eficientes.
...
- Agregar variables tiene dos efectos: disminuyen el sesgo, y también puede 
reducir el error irreducible.
- Omitir variables o simplificar estos modelos disminuye típicamente la varianza 
pero puede aumentar el sesgo.
:::
...
Como discutimos al final de la sección anterior, minimizar directamente el error
 ...
 de funciones, penalizar la función objetivo, perturbar la muestra de entrenamiento, o cambiar el proceso de minimización perturbando la función objetivo en cada paso o deteniendo el proceso antes de llegar a un mínimo sobreajustado.
...
de área habitable dependiera de la calidad de los terminados (@sec-medicioncostosa).
...
3.  Sustituimos variables categóricas con $K$ niveles en $K-1$ columnas indicadoras de los niveles (estableciendo) alguna categoría como referencia. Esto no es 
estrictamente necesario en otros métodos, o 
si utilizamos regularización (ver sección siguiente).
...
de interacciones en el ejemplo de las casas (@sec-medicioncostosa) y en el primer
...
cuantos valores de entrenamiento (intenta explicar de dónde sale esa varianza). 
Una solución puede ser transformar la entrada por
ejempo usando el logaritmo, que comprime la cola derecha de la distribución
de la variable que tiene mucho sesgo, y utilizar métodos como splines (ver más adelante) para limitar el
efecto de los datos atípicos sobre el grueso de las predicciones:
...
- Probablemente tendremos que agregar más flexibilidad en nuestro
predictor para capturar apropiadamente la información en esta variable, como mostramos
en la gráfica anterior. Esto muestra que con flexibilidad apropiada, la existencia de atípicos
no necesariamente afecta mucho la estimación donde existe la mayoría de los valores.
- En algunos casos, si después de investigar estas observaciones vemos que son datos
erróneos (y conocemos la razón del error o medición defectuosa), podemos corregirlos o eliminarlos.
Esto es parte del preprocesamiento y es necesario también probar el impacto en desempeño de estas
modificaciones.
...
Para entender mejor esta construcción, primero consideramos el uso de polinomios
para hacer expansión de entradas. Por ejemplo, si escogemos grado 3, la idea es convertir
una variable $x$ en varias variables $x, x^2, x^3, x^4$, de modo que nuestro
modelo original

$$f(x) = a + bx + \textrm{otros términos}$$
Se convierte en 

$$f(x) = a + b_1x + b_2x^2 + b_3x^3 + b_4x^4 + \textrm{otros términos}$$

Para hacer esto, sólo tenemos que precalcular $x, x^2, x^3, x^4$ y añadir al modelo
lineal.

En el caso de splines, con 4 grados de libertad por ejemplo, el modelo se convierte en:

$$f(x) = a + b_1s_1(x) + b_2s_2(x) + b_3s_3(x) + b_4s_4(x) + \textrm{otros términos}$$
donde cada $s_i(x)$ es una función cúbica por tramos que vale 0 fuera de un intervalo.
Igual que con polinomios, estas variables $s_i(x)$ pueden precalcularse a partir
de la $x$ original y los nudos que escogimos. Puedes ver los detalles en
nuestra referencia @ESL.
...
- Evitan el problema de la naturaleza global de los polinomios, 
en los que cambios en una parte de los datos puede afectar severamente la forma del polinomio en otros lugares lejanos. Esto limita los efectos negativos de la varianza adicional en
las colas de los datos (por ejemplo atípicos).
...
Vamos a separar en muestra de entrenamiento y de prueba estos datos. Podemos
