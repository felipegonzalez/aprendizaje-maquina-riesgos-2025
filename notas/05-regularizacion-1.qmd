...
considera un modelo lineal como "simple" o de "baja complejidad", es posible que la variabilidad de las estimaciones sea  grande y sobreajustar.

En este caso, minimizar el error cuadrático medio sobre la muestra de entrenamiento
como vimos en la sección anterior puede no ser muy buena idea. En esta parte veremos
una estrategia básica para "limitar" el grado de ajuste que nuestro modelo lineal puede alcanzar con datos de entrenamiento, esperando que esto mejore el desempeño predictivo. En otras
palabras, modificamos nuestra función objetivo para que las $f(x)$ 
no sobreajusten a las $y$. Este primera estrategia es llamada **regularización**,
y consiste agregar una penalización por tamaño de los coeficientes a la función objetivo.
...
con ciertos pesos o coeficientes $\beta$ que fijaremos.
...
- (Quitar unidades) Normalmente normalizamos las variables de entrada $x$ para que tenga sentido
penalizar todos los coeficientes con una misma $\lambda$.
...
g_l2 <- ggplot(coefs, aes(x = lambda, y = estimate)) +
  geom_line(aes(colour = term), size = 1.4) + scale_x_log10() +
...
![Ridge vs Lasso (Hastie, Tibshirani y Friedman)](figuras/ridge_lasso.png)
...
Descenso en gradiente estocástico es quizá uno de los métodos de ajuste
más importantes en el aprendizaje automático moderno, porque al mismo tiempo que da regularización, permite escalar
a problemas de optimización grandes, como los que surgen en redes neuronales.
...
no es muy diferente a $f_0$, utilizando información del gradiente en algunos puntos
de entrenamiento. En lugar de seguir iterando hasta llegar
...
sobreajustado (como vimos en regularización L2 y L1).
...
- En cada iteración $i$ hacemos un salto relativamente chico en una dirección de descenso
...
## keras tiene distintos algoritmos de optimización
## nótese que una capa densa equivale a regresión lineal sobre las entradas
...
g_dest <- pesos_tbl |> pivot_longer(cols  = -contains(c("grasacorp", "epoca"))) |> 
  rename(term = name) |> 
  ggplot(aes(x = epoca, y = value)) + 
  geom_line(aes(colour = term), linewidth = 1.4) +   scale_colour_manual(values = cbb_palette) +
...
*early stopping*). Sus ventajas son:

- Al cambiar la función de pérdida en cada paso, es posible escapar
de puntos estacionarios subóptimos (si el problema tiene varios puntos
estacionarios, es decir, donde el gradiente es cero). Evitamos mínimos locales 
sobreajustados: estos desaparecen cuando
consideramos "lotes" o "batches" de datos, en lugar del conjunto completo para 
cada iteración.

- Generalmente detenemos las iteraciones cuando el error de validación
deja de disminuir, lo cual es una forma de regularización que mantiene los parámetros
en valores relativamente chicos.
...
Finalmente, notamos que este tipo de regularización resulta en comportamientos
a veces inesperados del desempeño predictivo en función del tamaño del modelo
utilizado, por ejemplo, puede ocurrir el fenómeno de doble descenso (ver @ISL):

1. Comenzando en modelos muy simples, al incrementar la complejidad o tamaño del
model el error de prueba disminuye.
2.  A partir de cierto momento, incrementar la complejidad incrementa el error
de prueba (como esperaríamos por un aumento en varianza). El mínimo, que puede ser único,
da una solución sobreajustada, posiblemente con parámetros "muy grandes".
3. Sin embargo, si continuamos incrementando la complejidad tanto que existen múltiples
mínimos para el error de entrenamiento, el error de prueba
vuelve a descender, pues el esquema de optimización logra encontrar un mínimo local
con mejor generalización y menos sobreajuste.

Esto se puede deber, por ejemplo, pues cuando existen pocos grados de libertad,
la solución es escencialmente única (del paso 1 a 2 de arriba). Sin embargo, con modelos más
grandes aparecen muchas otras soluciones posibles: las mejores en entrenamiento son
peores en generalización, pero las que son "regularizadas" por
descenso estocástico son mejores en desempeño predictivo. Puedes pensar en un polinomio
de grado muy alto: la solución de grado alto que encontramos con descenso estocástico
puede ser muy suave, en contraste con lo que encontraríamos si llegáramos a un mínimo
