...
En todos los casos hasta ahora, hemos visto cómo

- Hacer predicciones puntuales (un solo valor). 
...
- Hacemos un gran número de predicciones, ninguna de ellas con costos relativos muy grandes,
...
individuales juegna un papel importante:
...
que no conocemos de antemano, y la estructura de costos de un error es compleja y sólo parcialmente definida. Ninguna pérdida usual refleja correctamente el costo-beneficio de usar un modelo.
...
$$[\hat{f}(\mathbf{x}) - q, \hat{f}(\mathbf{x}) + q]$$

Para esta nueva observación $(\mathbf{x}, \mathbf{y})$,

$$P(\mathbf{y} \in[\hat{f}(\mathbf{x}) - q, \hat{f}(\mathbf{x}) + q])\gtrsim 0.90$$
...
Estos intervalos pueden ser menos poco útiles
