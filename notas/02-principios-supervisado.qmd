...
este error seleccionando nuestro método de predicción $\hat{f}$ (y el tamaño de la muestra de entrenamiento)  de manera adecuada.
...
## Error cuadrático medio y descomposición sesgo-varianza

En la descomposición general que vimos arriba es difícil obtener un 
resumen que caracterize el error de predicción para cada $x$ sobre todas
las muestras de entrenamiento. Con el **error cuadrático**, sin embargo,
podemos hacer los siguiente:

El error de predicción para una $x$ particular es

$$ E_{\mathbf{y}|\mathbf{x}}[(\mathbf{y} - \hat{f_{\mathcal{L}}}(\mathbf{x}))^2]$$

donde la esperanza es sobre la condicional de $Y$ dado $X=x$. Esta cantidad
es igual a (demostrar):

$$ E_{\mathbf{y}|\mathbf{x}}[(\mathbf{y} - f^*(\mathbf{x}))^2] + (\hat{f_{\mathcal{L}}}(\mathbf{x}) - f^*(\mathbf{x}))^2=
\sigma^2(\mathbf{x}) + (\hat{f_{\mathcal{L}}}(\mathbf{x}) - f^*(\mathbf{x}))^2.$$

El primer término es **error irreducible** (pues no depende de $\hat{f_{\mathcal{L}}}$), y el segundo es el **error reducible**. 
Podemos resumir aún más si consideramos promediar sobre todas las posibles muestras de entrenamiento $\mathcal{L}$. El segundo término
se puede escribir como:


$$ E_{\mathcal{L}}[(\hat{f_{\mathcal{L}}}(\mathbf{x}) - f^*(\mathbf{x}))^2] = \text{Var}_{\mathcal{L}}(\hat{f_{\mathcal{L}}}(\mathbf{x})) + \text{Sesgo}_{\mathcal{L}}(\hat{f_{\mathcal{L}}}(\mathbf{x}))^2$$

De modo que obtenemos la siguiente descomposición del error cuadrático medio, que proviene de la teoría estádistica clásica:

::: callout-note
# Descomposición sesgo-varianza

$$ E_{\mathbf{y}|\mathbf{x}}[(\mathbf{y} - \hat{f_{\mathcal{L}}}(\mathbf{x}))^2] = \sigma^2(\mathbf{x}) + \text{Sesgo}^2_{\mathcal{L}}(\hat{f_{\mathcal{L}}}(\mathbf{x}))+ \text{Var}_{\mathcal{L}}(\hat{f_{\mathcal{L}}}(\mathbf{x})) .$$

:::

En este caso:

- El error irreducible es la varianza condicional de $y$ dada $x$.
- El sesgo mide qué tan lejos está la predicción promedio de la predicción óptima $E[y|x]$.
- La varianza es una medición de qué tanto cambia la predicción con la muestra de entrenamiento.

Nótese que para cada $x$, esta descomposición es más simple y nos da tres términos resumen contribuyen aditivamente para explicar 
el error de predicción promedio (no depende de la muestra de entrenamiento), y aunque no se generaliza para otras pérdidas más
generales, también provee una manera de entender cómo ocurre sobreajuste y subajuste.
