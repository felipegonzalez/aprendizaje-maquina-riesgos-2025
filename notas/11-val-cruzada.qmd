...
Recordamos los papeles de las muestras de entrenamiento, validación y
prueba:
...
Ver la discusión de la sección 2.
...
simular_evals <- function(reps, flujo, beta){
  ...
  eval_prueba <- predict(flujo_ajustado, datos_pr) |> 
    bind_cols(datos_pr |> dplyr::select(y)) |> 
    metricas(y, .pred)
...
```{r}
set.seed(82853)
evals_tbl <- tibble(reps = 1:25) |> 
  mutate(data = map(reps, ~ simular_evals(.x, flujo_1, beta = a))) |> 
  unnest(data)
```
...
particion_val <- initial_validation_split(datos, c(0.05, 0.9))
candidatos <- tibble(penalty = exp(seq(-5, 5, 1)))
# evaluar
val_datos <- tune_grid(flujo, resamples = validation_set(particion_val), grid = candidatos,
                       metrics = metric_set(rmse, mae)) |> 
  collect_metrics() |> 
...
datos_ent <- training(particion_val)
